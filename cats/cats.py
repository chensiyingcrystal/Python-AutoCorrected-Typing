...
def choose(paragraphs, select, k):
    ...
    Arguments:
        ...
        k: an integer"""
    # BEGIN PROBLEM 1
    paralist = [p for p in paragraphs if select(p)]
    return paralist[k] if k < len(paralist) else ''
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    if (typed_words == [] and reference_words != []) or (typed_words != [] and reference_words == []):
        return 0.0
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        d = {v : diff_function(typed_word, v, limit) for v in valid_words}
...
# big_limit = 10
# print(feline_flips("nice", "rice", big_limit))    # Substitute: n -> r
#     # 1
# print(feline_flips("range", "rungs", big_limit))  # Substitute: a -> u, e -> s
#     # 2
# print(feline_flips("pill", "pillage", big_limit)) # Don't substitute anything, length difference of 3.
#     # 3
# print(feline_flips("roses", "arose", big_limit))  # Substitute: r -> a, o -> r, s -> o, e -> s, s -> e
#     # 5
# print(feline_flips("rose", "hello", big_limit))   # Substitute: r->h, o->e, s->l, e->l, length difference of 1.
#     # 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == '':  # Fill in the condition
        ...
        remove = 1 + minimum_mewtations(start[1:], goal[1:], limit - 1)
        substitute = 1 + minimum_mewtations(start[1:], goal, limit - 1)
        return min(add, remove, substitute)
        # END
# big_limit = 10
# print(minimum_mewtations("cats", "scat", big_limit))      # cats -> scats -> scat
#     # 2
# print(minimum_mewtations("purng", "purring", big_limit))   # purng -> purrng -> purring
#     # 2
# print(minimum_mewtations("ckiteus", "kittens", big_limit)) # ckiteus -> kiteus -> kitteus -> kittens
#     # 3
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(0, len(sofar)):
        if sofar[i] == prompt[i]:
            ...
            break
    d = {'id': user_id, 'progress': correct_words / len(prompt)}
    upload(d)
    return d['progress']
    # END PROBLEM 8
...
# print(report_progress(sofar, prompt, 2, print_progress))
#     # ID: 2 Progress: 0.6
#     # 0.6
# print(report_progress(['how', 'aree'], prompt, 3, print_progress))
#     # ID: 3 Progress: 0.2
#     # 0.2
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    times = []
    for n in range(0, len(times_per_player)):
        times.append([times_per_player[n][i + 1] - times_per_player[n][i] for i in range(0, len(times_per_player[n]) - 1)])
    return [words, times]
    # END PROBLEM 9
...
# print(get_words(match))
#     # ['collar', 'plush', 'blush', 'repute']
# print(get_times(match))
#     # [[6, 3, 6, 2], [10, 6, 1, 2]]
...
def fastest_words(match):
    ...
    for i in word_indices:
            list_word_i = {n: time(match, n, i) for n in player_indices}
            player_list[min(list_word_i, key = list_word_i.get)].append(word_at(match, i))
        # if time(match, 0, i) < time(match, 1, i):
        #     player0_word_list.append(word_at(match, i))
        # elif time(match, 0, i) == time(match, 1, i):
        #     player0_word_list.append(word_at(match, i))
        # else:
        #     player1_word_list.append(word_at(match, i))
    return player_list
    # END PROBLEM 10
...
# # [['have', 'fun'], ['Just']]
# print(p0) # input lists should not be mutated
#     # [5, 1, 3]
# print(p1)
#     # [4, 1, 6]
